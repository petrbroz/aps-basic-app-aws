...
The application uses several AWS resources, including Lambda functions, an API Gateway API, and an S3 Bucket with a CloudFront Distribution. These resources are defined in the `template.yaml` file in this project. You can update the template to add AWS resources through the same deployment process that updates your application code.
...
![Serverless API](./docs/serverless-api.png)
...
![Full-Stack](./docs/full-stack.png)
...
For convenience, the included `deploy_frontend.sh` bash script can be run to automatically deploy your front end website to your AWS account:
...
## Use the AWS SAM CLI to build and run locally
...
The AWS SAM CLI installs dependencies that are defined in `backend/package.json`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
sam delete --stack-name aps-basic-app-aws
```
